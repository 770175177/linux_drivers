...
struct imx6uirq_dev
{
	...
	wait_queue_head_t r_wait;   /* head of wait queue */
...
*/
static irqreturn_t
key0_handler (int irq, void *dev_id)
{
...
timer_function (unsigned long arg)
{
	...
	if (atomic_read(&dev->releasekey))  /* complete a key input-release */
	{
		if (dev->async_queue)
		{
...
static imx6uirq_release(struct inode *node, struct file *filp)
{
	...
	for (i = 0; i < KEY_NUM; i++)
	  {
		  ...
		  imx6uirq.irqkeydesc[i].irqnum =
			  irq_of_parse_and_map (imx6uirq.nd, i);
#if 0
		  imx6uirq.irqkeydesc[i].irqnum =
			  gpio_to_irq (imx6uirq.irqkeydesc[i].gpio);
#endif
...
*/
static int
imx6uirq_open (struct inode *inode, struct file *filp)
{
...
*/
static ssize_t
imx6uirq_read (struct file *filp, char __user * buf,
			   size_t cnt, loff_t * offt)
{
	...
	if (atomic_read(&dev->releasekey) == 0)  /* not press key */
	{
		add_wait_queue(&dev->r_wait, &wait); /* add to wait queue */
		__set_current_state(TASK_INTERRUPTIBLE); /* set task state */
		schedule();
		if (signal_pending(current))         /* is signal wakeup */
		{
			ret = -ERESTARTSYS;
			goto wait_error;
		}
		__set_current_state(TASK_RUNNING);   /* set run state */
		remove_wait_queue(&dev->r_wait, &wait); /* remove to queue */
	}
#endif
...
wait_error:
#if 0
	set_current_state(TASK_RUNNING);       /* set task state */
...
unsigned int imx6uirq_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	if (atomic_read(&dev->releasekey))  /* key pressed */
	{
		mask = POLLIN | POLLRDNORM;     /* return PLLIN */
	}
