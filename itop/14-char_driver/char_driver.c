...
/*include device_create struct class and so on*/
...
static int chardevnode_open(struct inode *node, struct file *fp)
{
...
static int chardevnode_release(struct inode *node, struct file *fp)
{
...
static long chardevnode_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
...
static ssize_t chardevnode_read(struct file *fp, char __user *buf, size_t len, loff_t *f_ops)
{
	return 0;
}

static ssize_t chardevnode_write(struct file *fp, const char __user *buf, size_t len, loff_t f_ops)
{
	return 0;
}

static loff_t chardevnode_llseek(struct file *fp, loff_t offset, int len)
{
	return 0;
}
...
static int cDev_init(void)
{
	...
	for (i = 0; i < DEVICE_MINOR_NUM; i++)
	{
		...
		/*create device node*/
		device_create(my_class, NULL, MKDEV(number_major, number_minor + i), NULL, DEVICE_NAME"%d", i);
	}
...
static void cDev_exit(void)
{
	...
	for (i = 0; i < DEVICE_MINOR_NUM; i++)
	{
		...
		device_destroy(my_class, MKDEV(number_major, number_minor + i));
	}
